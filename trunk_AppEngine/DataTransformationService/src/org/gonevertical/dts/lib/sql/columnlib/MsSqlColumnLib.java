...
public class MsSqlColumnLib implements ColumnLib {
  ...
  private static final Logger log = LoggerFactory.getLogger(MsSqlColumnLib.class);
    ...
    for (int i=0; i < columnData.length; i++) {
      ...
      if (columnData[i].isFunctionSetForValue() == true) {
        ...
        if (columnData[i].getValue() == null) {
          ...
          String s = new MsSqlQueryLib().escape(columnData[i].getValue());
          v = "'" + s + "'";
        }
      }
  ...
  public String getSql_IdentitiesIndex(DatabaseData dd, ColumnData[] columnData) {
    ...
    for (int i=0; i < columnData.length; i++) {
      if (columnData != null && columnData[i].getIdentityUse() == true) {
        ...
        if (columnData[i].getIdentityUse() == true && columnData[i].getSqlType().toLowerCase().contains("text")) {
